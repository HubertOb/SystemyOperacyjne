...
void *foo(void *ind)
{
    ...
    int start=((Args*) ind)->start;
    int stop=((Args*) ind)->stop;
    char *background=bg;
    char *foreground=fg;
    char* tmp;
...
pthread_t *init_threads(char* foreground, char* background, int threads_count)
{
    ...
    int a=(grid_height*grid_width)/threads_count;
    int r=(grid_height*grid_width)%threads_count;
    ...
    Args args[threads_count];
    for (int i=0;i<threads_count;i++){
        args[i].start=start;
        args[i].stop=stop;
        pthread_create(&threads[i],NULL, foo,(void *)&args[i]);
        start=stop;
        if(i+1<r){
            stop=start+a+1;
        }
