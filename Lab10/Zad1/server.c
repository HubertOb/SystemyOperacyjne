...
int main(int argc, char*argv[]) {
    ...
    const int servSocket=socket(AF_INET,SOCK_STREAM,0);
    const int unixServSocket=socket(AF_UNIX,SOCK_STREAM,0);

    bind(servSocket,(struct sockaddr *)&netSocket,sizeof(netSocket));
    bind(unixServSocket,(struct sockaddr*)&server,sizeof(server));
    listen(servSocket,15);
    listen(unixServSocket,15);
    ...
    while(true){
        ...
        for(int i=0;i<countOfEvents;i++){
            if(events[i].data.fd==unixServSocket){
                clientSocket= accept(unixServSocket,NULL,NULL);
                if(clientSocket!=-1){
                    recv(clientSocket,buffer, sizeof(buffer),0);
                }
            }
            else if(events[i].data.fd==servSocket){
                clientSocket= accept(servSocket,(struct sockaddr*)&client, &len);
                if(clientSocket!=-1){
                    recv(clientSocket,buffer, sizeof(buffer),0);
                }
            }
                ...
                if(numberOfClients<=15){
                    for (int i=0;i<15;i++){
                        if(clientsID[i]==-1){
                            ...
                            send(clientsSockets[i],buffer, sizeof(buffer),0);
                            break;
                        }
                    }
                }
